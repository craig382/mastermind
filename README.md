...
- nPegCombos4 = 14. botPerfect[][]: string and boardPerfect[]: string. Perfect guesses in the log by the board and the bot will be underlined. Also, the bot will look for and log all perfect guesses, not just the first one. The bot will open up guessArray to allCodes when 2 < validCodesIn.length <= nPegCombos4. Because it is impossible to find a perfect guess when there are more than 14 (nPegCompbos4) validCodesIn.

...
Mastermind.Turn_view = Backbone.View.extend({
    ...
    holeClicked: function (e) {
        // log('holeClicked');
        // @ts-ignore
        var hole_index = $(e.currentTarget).attr('id'), color_class = Mastermind.GameView.allPiecesView.getNub();
    ...
    codeIsValid: function () {
        var code_complete = false, code_array = this.model.get('code'), num_holes = code_array.length;
...
Mastermind.AllPieces_view = Backbone.View.extend({
    ...
    nubClicked: function (e) {
        // log('nubClick');
        // @ts-ignore
        var nub_class = $(e.currentTarget).attr('class').split(' ')[1];
...
Mastermind.Game_view = Backbone.View.extend({
    ...
    getCurrentTurn: function () {
        var turn_index = this.model.get('num_turns') - this.model.get('turns_remaining'), cur_turn = this.turns.at(turn_index);
        return cur_turn;
    },

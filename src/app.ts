...
var palletBbm: PalletModel;
var palletBbv: PalletViewInstance;
...
type MastermindRoot = {
    ...
    Pallet?: PalletModelConstructor;
    PalletView?: PalletViewConstructor;
...
class MastermindUtilities {
     ...
     * all turn hints. */
    updateTurnHints(allTurns = false): void {
        var minTurn = allTurns ? 0 : boardPegs.length - 1;
        for (var t = minTurn; t < boardPegs.length; t += 1) {
            var hint_string: string = '<p class="hint b">' + boardPegs[t][0] 
                + '</p><p class="hint w">' + boardPegs[t][1] + '</p>';
            if (validCountHints) hint_string += '<p class="hint c">' 
                + codeTree[t][boardGuesses[t]][boardPegs[t]].length + '</p>';
            turnsBbm[t].set('hint_string', hint_string);
        }
    }

    isValid(guessTurnIndex: number, guess: string): boolean {
        if (guessTurnIndex === 0) return true;
    ...
    isPerfect(guessTurnIndex: number, guess: string): boolean {
        if (guessTurnIndex === 0) return false;
        if (!boardGuesses[guessTurnIndex - 1]) 
            throw new Error(`isPerfect ERROR: boardGuesses[${guessTurnIndex - 1}] is undefined.`);
        if (!codeTree[guessTurnIndex][guess])
            throw new Error(`isPerfect ERROR: codeTree[${guessTurnIndex}][${guess}] is undefined.`);
        var nPegCombos = Object.keys(codeTree[guessTurnIndex][guess]).length;
        var lastTurnPegs = boardPegs[guessTurnIndex - 1];
        var validCodes = codeTree[guessTurnIndex - 1][boardGuesses[guessTurnIndex - 1]][lastTurnPegs];
...
type PalletModelConstructor = new () => PalletModel;
type PalletViewConstructor = new () => PalletViewInstance;
...
function createPalletView(): PalletViewInstance {
    var PalletViewClass = required(mm.PalletView);
    return new PalletViewClass();
}

function createPalletModel(): PalletModel {
